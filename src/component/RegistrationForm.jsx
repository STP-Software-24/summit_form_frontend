...
const RegistrationForm = () => {
  ...
  const inputs = [
    {
      ...
      type: "text",
      errorMessage:
        "Your Fullname should be 6-55 characters and shouldn't include any special character",
      label: "Full Name",
      required: true,
      validation: (value) => {
    ...
    },
    {
      ...
      type: "text",
      errorMessage:
        "University should be 3-40 characters and shouldn't include any special character",
      label: "University",
      required: true,
      validation: (value) => {
    ...
    },
    {
      ...
      type: "text",
      errorMessage:
        "Faculty should be 3-40 characters and shouldn't include any special character",
      label: "Faculty",
      required: true,
      validation: (value) => {
        if (value.length < 3 || value.length > 40) {
  ...
  const handleSubmit = async (e) => {
    ...
    //
    console.log(values);
    const url = "https://alert-tuna-tutu.cyclic.app/summit/attendees";
    //const url = "http://localhost:3000/summit/attendees";
    // check if user exists in Database
    try {
      const response = await axios.get(
        url + `/${encodeURI(values.email)}`,
        values
      );
      notifyAlreadyRig();
    } catch (error) {
      ...
      if (error.response.request.status == 404) {
        await axios.post(url, { attendee: values }).then((response) => {
  ...
  return (
    <>
      <div className="m-4 d-block d-md-inline ">
        <img
          src="/img/logo.svg"
          className=" m-0 mx-auto d-block d-md-inline-block "
          alt=""
        />
      </div>
      <div className="m-4 d-block d-md-inline ">
      ...
      <div className="app">
        ...
        <div className="me-2 p-2 w-100">
          <form
            ...
            className="group p-3 rounded-2 colorbody h-75 m-5"
          >
            {inputs.map((input) => (
              <FormInput
                key={input.id}
            ...
            <div className="text-center p-2">
              <button
                className="form-btn rounded-4 mt-2 w-40 btn-primary submit-button"
                type="submit"
              >
                SUBMIT
              </button>
